...
public class Server implements Runnable , ICallBack{
	...
	private Permbajtja_letres[] tableCards;
	...
	public Server(int listeningPort) throws IOException {
		...
		this.tableCards = new Permbajtja_letres[4];
	...
	public void CallBack(String Data , Object Caller) {
		...
		if(Flag.equals("C_RNM"))
		{
			...
			if(connectedNodes.size() == 4)
			{
				...
				Logic.sleep(2000);
				ArrayList<ArrayList<Permbajtja_letres>> Cards = Logic.generatePlayersCardSets();
				
				for(int i =0 ;i<4; i++)
				{
					ArrayList<Permbajtja_letres> clst = Cards.get(i);
		...
		else if (Flag.equals("C_CPD"))
		{
			...
			tableCards[indx] = new Permbajtja_letres(value,suit);
			if(cardCount == 0)
			{
				tableSuite = suit;
